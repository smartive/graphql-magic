...
import { applyUserFilters } from './filters';
import { applyUserSelects } from './selects';
import { buildAnotherObjectQuery, applyAnotherObjectSubQueries } from '../another-object/resolver';
import { buildSomeObjectQuery, applySomeObjectSubQueries } from '../some-object/resolver';
import { buildReactionQuery, applyReactionSubQueries } from '../reaction/resolver';

export const buildUserQuery = async (
  ...
  const query = node.ctx.knex.fromRaw(`"User" as "${node.ctx.aliases.getShort(node.resultAlias)}"`);

  const joins: Joins = [];
  await applyUserFilters(node, query, joins);
  applyUserSelects(node, query, joins);
  applyJoins(node.ctx.aliases, query, joins);

  const tables: [string, string][] = [
    ['User', node.rootTableAlias],
...
export const applyUserSubQueries = async (
  ...
  for (const subField of node.selectionSet.filter(isFieldNode).filter(({ selectionSet }) => selectionSet)) {
    ...
    switch (fieldName) {
      case 'deletedAnotherObjects': {
        ...
        const subFieldDef = summonByKey(node.ctx.info.schema.getType('User')?.astNode?.fields || [], 'name.value', fieldName);
        ...
        const queries = ids.map((id) =>
          subQuery.clone()
            .select(`${shortTableAlias}.deletedById as ${shortResultAlias}__deletedById`)
            .where({ [`${shortTableAlias}.deletedById`]: id }),
        );
        ...
        for (const child of children) {
          for (const entry of entriesById[child['deletedById'] as string] || []) {
      ...
      case 'createdManyObjects': {
        ...
        const queries = ids.map((id) =>
          subQuery.clone()
            .select(`${shortTableAlias}.createdById as ${shortResultAlias}__createdById`)
            .where({ [`${shortTableAlias}.createdById`]: id }),
        );
        ...
        for (const child of children) {
          for (const entry of entriesById[child['createdById'] as string] || []) {
            const childClone = cloneDeep(child);
            childClone.PARENT = entry;
            (entry[fieldAlias] as Entry[]).push(childClone);
          }
      ...
      case 'updatedManyObjects': {
        ...
        const queries = ids.map((id) =>
          subQuery.clone()
            .select(`${shortTableAlias}.updatedById as ${shortResultAlias}__updatedById`)
            .where({ [`${shortTableAlias}.updatedById`]: id }),
        );
        ...
        for (const child of children) {
          for (const entry of entriesById[child['updatedById'] as string] || []) {
            const childClone = cloneDeep(child);
            childClone.PARENT = entry;
            (entry[fieldAlias] as Entry[]).push(childClone);
          }
...
export const resolveUser = async (
  ...
  const { query, verifiedPermissionStacks } = await buildUserQuery(node);

  if (ctx.info.fieldName === 'me') {
    if (!ctx.user?.id) {
      return undefined;
    }
    void query.where({ [`${ctx.aliases.getShort('User')}.id`]: ctx.user.id });
  }
  ...
  await applyUserSubQueries(node, res, verifiedPermissionStacks);
