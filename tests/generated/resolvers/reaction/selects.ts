...
import { applyReviewSelects } from '../review/selects';
import { applyQuestionSelects } from '../question/selects';
import { applyAnswerSelects } from '../answer/selects';

export const applyReactionSelects = (
  ...
  void query.select(`${rootShort}.type as ${resultShort}__${TYPE_ALIAS}`);
  ...
  for (const fragment of node.selectionSet.filter(isInlineFragmentNode)) {
    ...
    switch (typeName) {
      case 'Review': {
        applyReviewSelects({
          ...node,
          tableAlias: node.tableAlias + '__Review',
          selectionSet: fragment.selectionSet!.selections,
        }, query, joins);
        break;
      }
      case 'Question': {
        applyQuestionSelects({
          ...node,
          tableAlias: node.tableAlias + '__Question',
          selectionSet: fragment.selectionSet!.selections,
        }, query, joins);
        break;
      }
      case 'Answer': {
        applyAnswerSelects({
          ...node,
          tableAlias: node.tableAlias + '__Answer',
          selectionSet: fragment.selectionSet!.selections,
        }, query, joins);
        break;
      }
  ...
  for (const fieldNode of node.selectionSet.filter(isFieldNode).filter(({ selectionSet }) => selectionSet)) {
    ...
    switch (fieldName) {
      case 'parent': {
        ...
        addJoin(joins, node.tableAlias, 'Reaction', subTableAlias, 'parentId', 'id');
        applyReactionSelects({
          ...node,
