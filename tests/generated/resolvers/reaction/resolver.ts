...
import { applyReactionFilters } from './filters';
import { applyReactionSelects } from './selects';
import { applyUserSubQueries } from '../user/resolver';
import { applyReviewSubQueries } from '../review/resolver';
import { applyQuestionSubQueries } from '../question/resolver';
import { applyAnswerSubQueries } from '../answer/resolver';

export const buildReactionQuery = async (
  ...
  const query = node.ctx.knex.fromRaw(`"Reaction" as "${node.ctx.aliases.getShort(node.resultAlias)}"`);

  const joins: Joins = [];
  await applyReactionFilters(node, query, joins);
  applyReactionSelects(node, query, joins);
  applyJoins(node.ctx.aliases, query, joins);

  const tables: [string, string][] = [
    ['Reaction', node.rootTableAlias],
...
export const applyReactionSubQueries = async (
  ...
  for (const subField of node.selectionSet.filter(isFieldNode).filter(({ selectionSet }) => selectionSet)) {
    ...
    switch (fieldName) {
      case 'childReactions': {
        ...
        const subFieldDef = summonByKey(node.ctx.info.schema.getType('Reaction')?.astNode?.fields || [], 'name.value', fieldName);
        ...
        const { query: subQuery, verifiedPermissionStacks } = await buildReactionQuery(subNode, parentVerifiedPermissionStacks);
        ...
        const queries = ids.map((id) =>
          subQuery.clone()
            .select(`${shortTableAlias}.parentId as ${shortResultAlias}__parentId`)
            .where({ [`${shortTableAlias}.parentId`]: id }),
        );
        ...
        for (const child of children) {
          for (const entry of entriesById[child['parentId'] as string] || []) {
  ...
  for (const fragment of node.selectionSet.filter(isInlineFragmentNode)) {
    const typeName = getFragmentTypeName(fragment);
    switch (typeName) {
      case 'Review': {
        await applyReviewSubQueries({
          ...node,
          selectionSet: fragment.selectionSet!.selections,
        }, entries, parentVerifiedPermissionStacks);
        break;
      }
      case 'Question': {
        await applyQuestionSubQueries({
          ...node,
          selectionSet: fragment.selectionSet!.selections,
        }, entries, parentVerifiedPermissionStacks);
        break;
...
export const resolveReaction = async (
  _parent: any,
  _args: any,
  partialCtx: Context,
  ...
  const node: ConcreteFieldNode = {
    ctx,
    rootTableAlias: 'Reaction',
    tableAlias: 'Reaction',
    resultAlias: 'Reaction',
  ...
  const { query, verifiedPermissionStacks } = await buildReactionQuery(node);

  if (ctx.info.fieldName === 'me') {
    if (!ctx.user?.id) {
      return undefined;
    }
    void query.where({ [`${ctx.aliases.getShort('Reaction')}.id`]: ctx.user.id });
  }
  ...
  await applyReactionSubQueries(node, res, verifiedPermissionStacks);
