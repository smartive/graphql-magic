...
import { applyQuestionSelects } from './selects';
...
export const buildQuestionQuery = async (
  ...
  const joins: Joins = [];
  await applyQuestionFilters(node, query, joins);
  applyQuestionSelects(node, query, joins);
...
export const applyQuestionSubQueries = async (
  ...
  for (const subField of node.selectionSet.filter(isFieldNode).filter(({ selectionSet }) => selectionSet)) {
    ...
    switch (fieldName) {
      case 'childReactions': {
        ...
        const subFieldDef = summonByKey(node.ctx.info.schema.getType('Question')?.astNode?.fields || [], 'name.value', fieldName);
...
export const resolveQuestion = async (
  ...
  const { query, verifiedPermissionStacks } = await buildQuestionQuery(node);
  ...
  await applyQuestionSubQueries(node, res, verifiedPermissionStacks);
