...
export const buildAnswerQuery = async (
  ...
  const joins: Joins = [];
  await applyAnswerFilters(node, query, joins);
  applyAnswerSelects(node, query, joins);
...
export const applyAnswerSubQueries = async (
  ...
  for (const subField of node.selectionSet.filter(isFieldNode).filter(({ selectionSet }) => selectionSet)) {
    ...
    switch (fieldName) {
      case 'childReactions': {
        ...
        const subFieldDef = summonByKey(node.ctx.info.schema.getType('Answer')?.astNode?.fields || [], 'name.value', fieldName);
...
export const resolveAnswer = async (
  ...
  const { query, verifiedPermissionStacks } = await buildAnswerQuery(node);
  ...
  await applyAnswerSubQueries(node, res, verifiedPermissionStacks);
