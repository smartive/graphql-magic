...
export const buildReviewQuery = async (
  ...
  const joins: Joins = [];
  await applyReviewFilters(node, query, joins);
  applyReviewSelects(node, query, joins);
...
export const applyReviewSubQueries = async (
  ...
  for (const subField of node.selectionSet.filter(isFieldNode).filter(({ selectionSet }) => selectionSet)) {
    ...
    switch (fieldName) {
      case 'childReactions': {
        ...
        const subFieldDef = summonByKey(node.ctx.info.schema.getType('Review')?.astNode?.fields || [], 'name.value', fieldName);
...
export const resolveReview = async (
  ...
  const { query, verifiedPermissionStacks } = await buildReviewQuery(node);
  ...
  await applyReviewSubQueries(node, res, verifiedPermissionStacks);
