...
export const generatePermissionTypes = (models: Models) => {
  ...
  sourceFile.addStatements((writer) =>
    writer.write(`export type PermissionsBlock = true | `).inlineBlock(() => {
      writer.writeLine(`me?: UserPermissionBlock,`);
      for (const model of models.entities) {
        writer.writeLine(`${model.name}?: ${model.name}PermissionBlock,`);
      }
    }),
  );
  ...
  for (const model of models.entities) {
    ...
    sourceFile.addStatements((writer) =>
      writer.write(`export type ${model.name}PermissionBlock = `).inlineBlock(() => {
        ...
        if (relations.length > 0) {
          writer.write(`RELATIONS?: `).inlineBlock(() => {
            for (const relation of relations) {
              writer.writeLine(`${relation.name}?: ${relation.targetModel.name}PermissionBlock,`);
            }
          });
        }
      }),
